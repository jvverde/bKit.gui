...
const { relative, normalize } = window.electron.path
...
export default {
  ...
  computed: {
    ...
    isBackupable () { return (this.onlyLocal || this.needUpdate) && !this.isForbidden },
    onBackupQueue () { return this.isQueued(this.fullpath) },
    showBackup () { return this.isLastSnap && this.isBackupable && !this.onBackupQueue },
    isRestorable () { return (this.needUpdate || this.onlyBackup) && this.mountpoint && !this.isForbidden },
  ...
  methods: {
    ...
    async recover () {
      ...
      const download = await window.electron.askUser4Location2recovery()
