...
import depends from './conf/depends'
...
export const isbkitClintInstalled = (location) => {
  return location && existsSync(location) && depends.every(e => {
    const fullpath = path.join(location, e)
    say.log(`Check if script ${fullpath} exists?`, existsSync(fullpath))
...
export const getbkitPath = () => {
  return bkitPath() || findbkit()
}
